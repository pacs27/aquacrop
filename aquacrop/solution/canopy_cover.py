...
def canopy_cover_in_growing_season(
    ...
    calculate_potential_canopy_cover()
    if (time_canopy_cover_adjusted < crop.Emergence) or (
        round(time_canopy_cover_adjusted) > crop.Maturity
    ):
        # No canopy development before emergence/germination or after
        # maturity
        NewCond.canopy_cover_ns = 0
    elif time_canopy_cover_adjusted < crop.CanopyDevEnd:
        # Canopy growth can occur
        if initial_cc_ns <= crop.CC0:
            # Very small initial canopy_cover.
            NewCond.canopy_cover_ns = crop.CC0 * np.exp(
                crop.CGC * time_delta_of_canopy_growth
            )
            # print(crop.CC0,np.exp(crop.CGC*time_delta_of_canopy_growth))
        else:
            # Canopy growing
            tmp_tCC = time_canopy_cover_adjusted - crop.Emergence
        ...
        if time_canopy_cover_adjusted < crop.Senescence:
    ...
    if (time_canopy_cover_adjusted < crop.Emergence) or (
        ...
        if initial_cc <= NewCond.cc0_adj or (
            ...
            if initial_protected_seed == True:
                ...
                NewCond.canopy_cover = NewCond.cc0_adj * np.exp(
                    crop.CGC * time_delta_of_canopy_growth
                )
                ...
                if CGCadj > 0:
                    ...
                    if CCXadj < 0:
                        ...
                        if tReq > 0:
                            ...
                            tmp_tCC = tReq + time_delta_of_canopy_growth
    ...
    if time_canopy_cover_adjusted >= crop.Emergence:
        if (time_canopy_cover_adjusted < crop.Senescence) or (initial_tEarlySen > 0):
            ...
            if (water_stress_coef.sen < 1) and (initial_protected_seed == False):
                ...
                NewCond.t_early_sen = initial_tEarlySen + time_delta_of_canopy_growth
                ...
                if (time_canopy_cover_adjusted > crop.Senescence) and (
                    initial_tEarlySen > 0
                ):
    ...
    if NewCond.canopy_cover_ns < NewCond.canopy_cover:
        ...
        if time_canopy_cover_adjusted < crop.CanopyDevEnd:
            NewCond.ccx_act_ns = NewCond.canopy_cover_ns
